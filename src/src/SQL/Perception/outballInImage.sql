...
(Timestamp-(select max(Timestamp) from profiling as ty where ty.Timestamp<tx.Timestamp and EventSubject like 'perception%out/ballInImage')) * (CASE EventType WHEN 'LOCK' THEN 0 ELSE 1 END)  as "Result(milisec)"
